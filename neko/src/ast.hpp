...
enum class ExprKind { IntLiteral, Identifier, Call };
...
struct Expr {
    ...
    static Expr make_int(int64_t v) {
        return { ExprKind::IntLiteral, v, {}, {} };
    }
    static Expr make_id(std::string n) {
        return { ExprKind::Identifier, 0, std::move(n), {} };
    }
    static Expr make_call(std::string callee, std::vector<Expr> a) {
        return { ExprKind::Call, 0, std::move(callee), std::move(a) };
    }
};
